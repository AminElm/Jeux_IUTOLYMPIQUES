...
epreuve_collective = ["Natation relais libre", "Handball", "Volley-Ball","Athlétisme relais 400m"]
...
for _ in range(400):
    ...
    sports = epreuve.split(" ")
    sport = sports[0]
    force = random.randint(1, 20)
    endurance = random.randint(1, 20)
    agilite = random.randint(1, 20)
    
    if epreuve in epreuve_collective:
        collectif = "T"
    else:
        collectif = "F"
    records.append([nom, prenom, sexe, pays_choice, sport, epreuve,collectif, force,endurance, agilite])

# Écriture des données dans un fichier CSV
with open('donnees.csv', 'w', newline='', encoding='utf-8') as file:
    writer = csv.writer(file)
    # Écriture de l'en-tête
    writer.writerow(["Nom", "Prénom", "Sexe", "Pays","sport","Épreuve","collectif","Force","Endurance","agilite"])
